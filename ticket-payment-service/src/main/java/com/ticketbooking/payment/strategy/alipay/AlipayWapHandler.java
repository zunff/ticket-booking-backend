...
/**
 * 支付宝手机网站支付（wap.pay）— 调 {@code pageExecute}，返回跳转 URL/表单。
 */
@Slf4j
@Component
@RequiredArgsConstructor
@ConditionalOnProperty(name = "payment.channels.alipay.enabled", havingValue = "true")
public class AlipayWapHandler implements PayModeHandler {
    ...
    public PayResponseDTO prepay(PayRequestQO request) {
        ...
        try {
            ...
            log.info("Alipay WAP prepay success: outTradeNo={}", request.getOutTradeNo());
            return PayResponseDTO.builder()
                    .payMode(payMode())
                    .payUrl(response.getBody())
                    .build();
        } catch (AlipayApiException e) {
            log.error("Alipay WAP prepay failed: outTradeNo={}", request.getOutTradeNo(), e);
